"""Connection lifecycle for Elastic Observability Connector."""
from __future__ import annotations
import json, uuid
from imperal_sdk import ActionResult
from elastic_observability_client import ElasticObservabilityClient
...
_SECRET = "elastic_observability_connections"
...
@chat.function(
    "connect_elastic_observability",
    "Connect Elastic Observability account via credentials.",
    action_type="write",
    chain_callable=True,
    event="elastic-observability-connector.connect_elastic_observability",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_elastic_observability(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Elastic Observability Connector."""
    ...
    record = {
        "id": cid,
        "label": params.label or "Elastic Observability Account",
...
@chat.function(
    "list_connections",
    "List connected Elastic Observability accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_elastic_observability",
    "Disconnect Elastic Observability account.",
    action_type="write",
    chain_callable=True,
    event="elastic-observability-connector.disconnect_elastic_observability",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_elastic_observability(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
